<?php
        ...
        // corespondent aux colonnes de notre doc
         class MyLine extends Line{
            protected $nom_objet;
            protected $texte;
            protected $adresse_image;
            protected $adresse_son;
            protected $position_image;
            protected $credits;
        }
        
        define("USER_MAIL", "owni.rfi@gmail.com");
        define("USER_PWD",  "m0td3p4ssb4n4l");
        define("DOC_NAME",  "textes_independances");

        // facultatif mais conseillée, la clé de l'article (pour l'atteindre)
         // si null, on utilise le doc name pour retrouver la clef
        define("DOC_KEY",   "t_XIRJRAMjCj-AtsoRpNqwQ");
        ...
        if(file_exists(DIR_CACHE.DOC_NAME.".json") && $diff <= ((60*60)*UP_AFTER) ):
            ...
            foreach($cellFeed as $cellEntery){
                ...
                // vérifie s'il n'y a pas eu de cellule vide
                $diff = $cellEntery->cell->getColumn() - $lastColumn;
                // Si on a sauté des colonnes (car elles sont vides)
                // on complète avec des valeurs null
                for($i = 0; $i < $diff - 1; $i++)
                    $line_buffer[] = null;

                $lastColumn = $cellEntery->cell->getColumn();
            ...
            // on ajoute le dernier pardis !
            $table[] = new MyLine( $line_buffer );

            // stocke quelques infos sur le doc
            $file  = '{"generated" : '.time(). ', '."\n";
            $file .= '"doc_name" : "'.DOC_NAME. '", '."\n";
            $file .= '"doc_key" : "'.$spreadsheetsKey. '", '."\n";
            $file .= '"data" : [';
            ...
            $i = 0; foreach($table as $line_buffer) {
                ...
                if($i++>0)
                    $file .= ','."\n";
                // encode chaque ligne au format json
